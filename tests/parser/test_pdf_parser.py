...
class TestPdfParser:
    ...
    @patch.dict(
        os.environ, {"AZURE_OPENAI_ENDPOINT": "test", "AZURE_OPENAI_API_KEY": "test"}
    )
    @patch("src.file_parser.pdf_parser.AzureChatOpenAI")
    def test_setup_image_describer_success_with_prompt(self, mock_azure, parser):
        """Test successful setup with custom prompt file."""
        parser.describe_images = True
        mock_azure.return_value = Mock()
        with patch("src.file_parser.pdf_parser.os.path.exists", return_value=True):
    ...
    @patch("src.file_parser.pdf_parser.AzureChatOpenAI")
    def test_setup_image_describer_no_prompt_file(self, mock_azure, parser):
        """Test setup without custom prompt file."""
        parser.describe_images = True
        mock_azure.return_value = Mock()
        with patch("src.file_parser.pdf_parser.os.path.exists", return_value=False):
            parser.setup_image_describer()
        assert parser.llm is not None
    ...
    @patch("src.file_parser.pdf_parser.AzureChatOpenAI")
    def test_setup_image_describer_prompt_errors(self, mock_azure, parser):
    ...
    @patch("src.file_parser.pdf_parser.AzureChatOpenAI", side_effect=ImportError())
    def test_setup_image_describer_import_error(self, mock_azure, parser):
    ...
    def test_setup_image_describer_general_error(self, mock_azure, parser):
    ...
    def test_describe_image_success(self, mock_b64, mock_exists, mock_image, parser):
    ...
    def test_get_content_for_llm_structured(self, parser):
        ...
        parser.structured_content = [{"page": 1, "text": "content", "images": []}]
        parser.tables = [{"page": 1, "json": "{}"}]

        with patch("src.file_parser.pdf_parser.clean_text", return_value="cleaned"):
            result = parser.get_content_for_llm()
