...
def test_upload_to_blob_success(mock_blob_service, mock_open_file, caplog):
    ...
    assert any("Plik 'path/to/file.mp3' wysłany do kontenera 'audio'" in r.message for r in caplog.records)
...
def test_upload_to_blob_exception_handling(mock_blob_service, mock_open_file, caplog):
    caplog.set_level("ERROR")
    mock_blob_service.from_connection_string.side_effect = Exception("Some Azure error")

    upload_to_blob("logs", "fake.txt", "blob.txt")

    assert any("❌ Błąd przy wysyłaniu do Azure Blob Storage" in r.message for r in caplog.records)
