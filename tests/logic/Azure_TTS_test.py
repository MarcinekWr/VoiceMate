...
class TestAzureTTSPodcastGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.speech_config_patcher = patch('azure.cognitiveservices.speech.SpeechConfig', 
    ...
    def test_generate_podcast_empty_dialog_data(self, mock_mkdtemp, mock_get_request_id):
        ...
        with patch.object(generator.logger, 'error') as mock_logger_error:
            ...
            mock_logger_error.assert_called_once_with("Nie przekazano danych dialogowych.")
    ...
    @patch('os.path.join', return_value='/tmp/podcast_test/podcast_test_request_123.wav')
    def test_generate_podcast_success(self, mock_join, mock_synthesizer_class, 
                                    mock_audio_config, mock_temp_file, 
                                    mock_mkdtemp, mock_get_request_id):
        ...
        with patch.object(generator, '_combine_segments', return_value=expected_output) as mock_combine:
    ...
    @patch('azure.cognitiveservices.speech.audio.AudioOutputConfig')
    @patch('azure.cognitiveservices.speech.SpeechSynthesizer')
    def test_generate_podcast_incomplete_segment(self, mock_synthesizer_class, 
                                               mock_audio_config, mock_temp_file):
        ...
        with patch.object(generator.logger, 'warning') as mock_logger_warning:
            with patch.object(generator, '_combine_segments', return_value='output.wav'):
                result = generator.generate_podcast_azure(dialog_data)
    ...
    @patch('azure.cognitiveservices.speech.audio.AudioOutputConfig')
    @patch('azure.cognitiveservices.speech.SpeechSynthesizer')
    def test_generate_podcast_synthesis_error(self, mock_synthesizer_class, 
                                            mock_audio_config, mock_temp_file):
        ...
        with patch.object(generator.logger, 'error') as mock_logger_error:
            with patch.object(generator, '_combine_segments', return_value='output.wav'):
                ...
                mock_logger_error.assert_called_with("Błąd syntezy dla segmentu 1")

    @patch('tempfile.NamedTemporaryFile')
    @patch('azure.cognitiveservices.speech.audio.AudioOutputConfig')
    @patch('azure.cognitiveservices.speech.SpeechSynthesizer')
    def test_generate_podcast_exception_handling(self, mock_synthesizer_class, 
                                                mock_audio_config, mock_temp_file):
        ...
        mock_synthesizer = Mock()
        mock_synthesizer.speak_text_async.side_effect = Exception("Test exception")
        mock_synthesizer_class.return_value = mock_synthesizer
        ...
        with patch('tempfile.NamedTemporaryFile'):
            with patch('azure.cognitiveservices.speech.audio.AudioOutputConfig'):
                with patch('azure.cognitiveservices.speech.SpeechSynthesizer'):
                    with patch.object(generator, '_combine_segments', return_value='output.wav'):
                        generator.generate_podcast_azure(dialog_data, progress_callback=progress_callback)
    ...
    def test_combine_segments_success(self, mock_exists, mock_wave_open):
        ...
        mock_wave_open.return_value.__enter__ = lambda x: wave_open_side_effect(None, None)
        mock_wave_open.side_effect = lambda file, mode: MagicMock(__enter__=lambda x: wave_open_side_effect(file, mode))
    ...
    @patch('os.path.exists', return_value=True)
    @patch('os.remove')
    def test_combine_segments_file_removal_success(self, mock_remove, mock_exists, mock_wave_open):
    ...
    @patch('os.remove')
    @patch('time.sleep')
    def test_combine_segments_permission_error_retry(self, mock_sleep, mock_remove, 
                                                   mock_exists, mock_wave_open):
    ...
    @patch('os.path.exists', return_value=True) 
    @patch('os.remove')
    def test_combine_segments_permission_error_max_retries(self, mock_remove, 
                                                         mock_exists, mock_wave_open):
        ...
        with patch.object(generator.logger, 'warning') as mock_logger_warning:
            with patch('time.sleep'):
                ...
                self.assertEqual(mock_logger_warning.call_count, 3)  # 2 retry warnings + 1 final warning

    @patch('wave.open')
    @patch('os.path.exists', return_value=True)
    @patch('os.remove')
    def test_combine_segments_other_remove_exception(self, mock_remove, mock_exists, mock_wave_open):
    ...
    def test_combine_segments_mixed_file_existence(self, mock_exists, mock_wave_open):
        ...
        temp_files = ['/tmp/chunk1.wav', '/tmp/nonexistent.wav', '/tmp/chunk2.wav']
        output_path = '/tmp/output.wav'
...
class TestAzureTTSPodcastGeneratorIntegration(unittest.TestCase):
    ...
    @patch('os.remove')
    @patch('os.path.join')
    def test_full_podcast_generation_flow(self, mock_join, mock_remove, mock_exists, mock_wave_open,
                                        mock_synthesizer_class, mock_audio_config,
                                        mock_temp_file, mock_get_request_id, mock_mkdtemp):
        ...
        result = generator.generate_podcast_azure(dialog_data, progress_callback=progress_callback)
