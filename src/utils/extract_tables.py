...
def extract_tables_from_pdf(pdf_path: str, pages: str = "all") -> List[Dict[str, Any]]:
    """
    Extract tables from PDF and return them as structured data.

    Args:
        pdf_path (str): Path to the PDF file
        pages (str): Page numbers to extract from (default: 'all')

    Returns:
        List[Dict]: List of dictionaries containing table data and metadata
    """
    try:
        tables = camelot.read_pdf(
            pdf_path, pages=pages, flavor="lattice", strip_text="\n"
        )
...
def clean_table_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cleaned_df = cleaned_df[
        ~cleaned_df.astype(str).apply(lambda x: x.str.strip()).eq("").all(axis=1)
    ]
...
def format_tables_for_llm(tables: List[Dict[str, Any]]) -> str:
    ...
    for table in tables:
        ...
        try:
            ...
            formatted_output.append(json.dumps(json_data, indent=2, ensure_ascii=False))
        except Exception:
...
def test_table_extraction(pdf_path: str):
    """Test function to demonstrate table extraction."""
    ...
    if tables:
        ...
        for table in tables:
            print(
                f"Table {table['table_id']}: {table['shape']} on page {table['page']}"
            )

        llm_content = format_tables_for_llm(tables)
        print("\n" + llm_content)
...
if __name__ == "__main__":
    test_pdf_path = "assets/oiak.pdf"
    test_table_extraction(test_pdf_path)
