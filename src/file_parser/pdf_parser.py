...
class PdfParser:
    """
    A class to parse files and extract text, images, and metadata with image descriptions.
    """
    ...
    def setup_image_describer(self):
        ...
        try:
            ...
            try:
                if os.path.exists(prompt_path):
                    ...
                    self.prompt_template = PromptTemplate.from_template(
                        "Describe this image in detail. Focus on text, charts, diagrams, and any important visual elements."
                    )
            except (OSError, UnicodeDecodeError) as e:
                logger.warning("Failed to load custom prompt, using default: %s", e)
                self.prompt_template = PromptTemplate.from_template(
                    "Describe this image in detail. Focus on text, charts, diagrams, and any important visual elements."
                )
            ...
            logger.warning(
                "Import error setting up image describer: %s. Check if langchain_openai is installed.",
                e,
            )
            self.describe_images = False
        except Exception as e:
            logger.warning(
                "Unexpected error setting up image describer: %s. Images will be extracted without descriptions.",
                e,
            )
            self.describe_images = False
    ...
    def describe_image(self, image_path: str) -> str:
        ...
        if not self.describe_images or not self.llm or not self.prompt_template:
        ...
        try:
            ...
            logger.error("Unexpected error describing image %s: %s", image_path, e)
            return f"Error generating description: {str(e)}"

    def parse_all(self) -> Dict[str, Any]:
        """
        Modified version of parse_all() to include table extraction and image descriptions.
        """
        try:
            ...
            for page_num in range(len(doc)):
                ...
                for img_index, image in enumerate(image_list):
                    try:
                        ...
                        image_data_base64 = base64.b64encode(pix.tobytes("png")).decode(
                            "utf-8"
                        )
                        ...
                        if self.describe_images:
                            logger.info("Describing image: %s", img_filename)
    ...
    def get_content_for_llm(self) -> str:
        """
        Modified version to include tables and image descriptions in LLM content per page.
        """
        ...
        for page in self.structured_content:
            ...
            if page["images"]:
                ...
                for image in page["images"]:
                    page_text += f"- Image {image['filename']} ({image['width']}x{image['height']}, {image['size_kb']}KB)\n"
    ...
    def save_metadata_json(self) -> str:
        ...
        try:
            with open(metadata_path, "w", encoding="utf-8") as file:
                json.dump(
                    self.metadata, file, indent=2, ensure_ascii=False, default=str
                )
        except OSError as e:
...
if __name__ == "__main__":
    # Example usage
    path = "assets/oiak2.pdf"
    output_directory = "extracted_content"

    try:
        parser = PdfParser(path, output_directory, describe_images=True)

        content = parser.parse_all()
        ...
        print(f"Pages: {content['metadata'].get('page_count', 'N/A')}")

        if content["metadata"]:
            metadata = content["metadata"]
            print(f"File size: {metadata.get('file_size_mb', 'N/A')} MB")
        ...
        if content["images"]:
            ...
            for image in content["images"]:
                print(
                    f"- {image['filename']}: {image.get('description', 'No description')[:100]}..."
                )

        llm_content = parser.get_content_for_llm()
        print(f"LLM-ready content length: {len(llm_content)} characters")

        report_path = parser.save_summary_report()
        print(f"Summary report saved to: {report_path}")

        metadata_path = parser.save_metadata_json()
        print(f"Metadata JSON saved to: {metadata_path}")

        llm_content_path = os.path.join(output_directory, "llm_content.txt")
        with open(llm_content_path, "w", encoding="utf-8") as file:
            file.write(llm_content)
        print(f"LLM content saved to: {llm_content_path}")
