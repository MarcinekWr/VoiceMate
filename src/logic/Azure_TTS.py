...
class AzureTTSPodcastGenerator:
    ...
    def generate_podcast_azure(
        ...
        if output_path is None:
            ...
            output_path = os.path.join(temp_dir, f"{self.dir_prefix}{request_id}.wav")
    ...
    def _combine_segments(self, temp_files: list, output_path: str) -> str:
        ...
        if not first_file:
            self.logger.error("No files to merge - no segments were generated.")
            return None
            ...
            if os.path.exists(temp_file):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        self.logger.error(f"Error deleting file {temp_file}: {e}")
                        break
