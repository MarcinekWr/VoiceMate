...
def generate_podcast_text(llm: AzureChatOpenAI, style: str, input_text: str, plan_text: str, ui_callback=None) -> str:
    ...
    try:
        ...
        logger.info(f"Podcast text generated successfully ({len(response.content)} characters)")
        if ui_callback:
            ui_callback(f"Podcast wygenerowany pomyślnie! ({len(response.content)} znaków)", "success")
